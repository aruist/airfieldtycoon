...
namespace OneP.InfinityScrollView
{
	...
	public class InfinityScrollView : MonoBehaviour {
		...
		public GameObject[] arrayCurrent = null;
		...
		public void Setup(){
			//Debug.Log ("content width: " + content.GetWidth ());
			if (prefab == null) {
			...
			for (int i = 0; i < itemGenerate; i++) {
				...
				if (!isInit)
                {
					...
					if(type==InfinityType.Vertical)
					{
						...
						if(rect!=null){
							...
							if (overrideWidth) 
							{
								rect.SetWidth (Mathf.Abs( content.GetWidth ()));
							}
						}
					}
                ...
                else
				{
					if (i < totalNumberItem) {
                        ...
                        {
#if SOFTCEN_DEBUG
                            Debug.Log("<color=red>Setup error: i: " + i.ToString() + ", totalNumberItem: " + totalNumberItem + ", listItem.Count: " + listItem.Count);
#endif
                        }
                    }
				}
        ...
        public void UpdateAllItems()
        {
            ...
            {
				FinishItem fi = listItem[i].GetComponent<FinishItem>();
				if (fi != null)
				{
					fi.UpdateItem();
				}
				//TrackItem ti = listItem[i].GetComponent<TrackItem>();
				//if (ti != null)
				//{
				//	ti.UpdateStars();
				//}
